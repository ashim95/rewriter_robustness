...
evaluation_dir=hf_models/$hf_model/textattack_evaluation/
...
do
    ...
    textattack attack --recipe $attack --model-from-huggingface $hf_model --dataset-from-huggingface $dataset --num-examples -1 --dataset-split $split  --log-to-csv $csv_file --model-type-to-evaluate hf_model | tee $log_file
done
